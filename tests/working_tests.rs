...
async fn test_queue_service_operations() {
    ...
    let message_id = service
        .send_message("service-queue", "Test message", Some(attributes), None)
        .await
...
async fn test_message_delete_and_restore() {
    ...
    service
        .create_queue("delete-test-queue")
        .await
        .expect("Failed to create queue");

    let message_id = service
        .send_message("delete-test-queue", "Delete test", None, None)
        .await
        .expect("Failed to send message");

    // Receive message to get receipt handle
    let received = service
        .receive_message("delete-test-queue")
        .await
        .expect("Failed to receive message");
    assert!(received.is_some());
